...
function CategoryPage() {
  ...
  useEffect(() => {
    const fetchCategoryData = async () => {
      ...
      setLoading(true);
      try {
        const response = await axios.get(`https://zenblog-live-api.onrender.com/api/posts?categoryId=${categoryId}`);
        if (Array.isArray(response.data)) {
          ...
          if (response.data.length > 0) {
            ...
            const catResponse = await axios.get(`https://zenblog-live-api.onrender.com/api/categories/${categoryId}`); // Assumes this endpoint exists
